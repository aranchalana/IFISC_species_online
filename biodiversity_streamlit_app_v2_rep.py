...
- AI-powered species data extraction using Claude API
...
class BiodiversityPipeline:
    ...
    def search_scopus(self, species: str, api_key: str, max_results: int = 25, start_year: int = 2015, end_year: int = 2025, inst_token: str = None) -> List[Dict]:
        ...
        if not api_key:
            st.write("  Skipping Scopus search (no API key)")
            return []
...
def main():
    ...
    if st.sidebar.button("Start Analysis", type="primary"):
        ...
        for i, species in enumerate(species_list):
            ...
            species_progress.progress((i + 1) / total_species)
        ...
        if all_search_results:
            st.success(f"**Analysis complete!** Found {len(all_search_results)} papers and extracted {len(all_species_data)} species records")
        else:
